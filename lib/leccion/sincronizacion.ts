...
export function crearSincronizador(opciones: OpcionesSincronizador): Sincronizador {
  ...
  function limpiar() {
    ...
    // Callar aquí no deja mudo a nadie más: el turno de palabra lo vigila
    // `lib/leccion/voz.ts`, y una cancelación de quien no lo tiene se ignora
    // allí. Ésa es la regla que impide que la pizarra, al recolocarse siguiendo
    // al tutor, le corte la frase a media palabra.
    try {
      locutor?.cancelar();
    } catch {
